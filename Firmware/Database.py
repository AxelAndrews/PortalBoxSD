...
class Database:
    ...
    def __init__(self, settings):
        ...
        # Email API path (can be the same endpoint with different mode)
        self.email_api_path = self.api_path  # Using the same API endpoint
    ...
    # New method for sending emails through the HTTP API
    def send_email(self, to_user_id, subject, body, email_type="standard"):
        """
        Sends an email through the HTTP API.
        
        Args:
            to_user_id: User ID to send the email to
            subject: Email subject
            body: Email body
            email_type: Type of email (standard, proxy, training)
            
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            print(f"Sending email to user ID {to_user_id}, type: {email_type}")
    ...
    # Methods to mimic the old Emailer functionality
    def send_user_email(self, auth_id, equipment_id=None, equipment_type=None, location=None):
        """
        Sends an email to a user who left their card in a machine
        """
        # Get user information (already implemented)
        user = self.get_user(auth_id)
        
        if user and user[0] and user[1]:
            # If equipment details weren't provided, try to get them
            if not equipment_type and hasattr(self, 'equipment_type_id'):
                equipment_type = self.get_equipment_type_name(self.equipment_type_id)
            
            if not equipment_id and hasattr(self, 'equipment_id'):
                equipment_id = self.equipment_id
                
            if not location and hasattr(self, 'location'):
                location = self.location
                
            # Get equipment name if we have the ID
            equipment_name = ""
            if equipment_id:
                equipment_name = self.get_equipment_name(equipment_id)
                
            # Construct email body
            body = f"{user[0]}, it appears you left your access card in a portal box"
            
            if equipment_type:
                body += f" for the {equipment_type}"
            
            if equipment_name:
                body += f" named {equipment_name}"
                
            if location:
                body += f" in the {location}"
                
            # Send the email
            subject = "Access Card left in PortalBox"
            return self.send_email(auth_id, subject, body, "standard")
        else:
            print("Could not send email - user information not available")
            return False
            
    def send_user_email_proxy(self, auth_id, equipment_id=None, equipment_type=None, location=None):
        """
        Sends an email to a user who left a proxy card in a machine
        """
        # Get user information (already implemented)
        user = self.get_user(auth_id)
        ...
        if user and user[0] and user[1]:
            ...
            # Construct email body
            body = f"{user[0]}, it appears you left a proxy card in a portal box"
            ...
            # Send the email
            subject = "Proxy Card left in PortalBox"
            return self.send_email(auth_id, subject, body, "proxy")
        else:
            print("Could not send email - user information not available")
            return False
            
    def send_user_email_training(self, trainer_id, trainee_id, equipment_id=None, equipment_type=None, location=None):
        """
        Sends an email to a trainer and trainee when a training card is left in a machine
        """
        # Get user information for both trainer and trainee
        trainer = self.get_user(trainer_id)
        trainee = self.get_user(trainee_id)
        
        if trainer and trainee and trainer[0] and trainee[0]:
            # If equipment details weren't provided, try to get them
            if not equipment_type and hasattr(self, 'equipment_type_id'):
                equipment_type = self.get_equipment_type_name(self.equipment_type_id)
            ...
            # Construct email body
            body = f"{trainee[0]} (trained by {trainer[0]}), it appears you left your card in a portal box"
            ...
            # Send the email - in this case we'll need to loop and send to both 
            # or modify the API to handle multiple recipients
            subject = "Training Card left in PortalBox"
            
            # For now, send to trainer first, then trainee
            result1 = self.send_email(trainer_id, subject, body, "training")
            result2 = self.send_email(trainee_id, subject, body, "training")
    ...
    # Helper method to get equipment type name
    def get_equipment_type_name(self, equipment_type_id):
        """
        Gets the name of an equipment type by ID
        """
